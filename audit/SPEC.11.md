...
11. **`anonymous_proxy` : gap d'autorisation UI-only (résolu — pas dead config).** Le flag gouverne bien l'affichage (`give.html.twig`, `card_action.html.twig` masquent/affichent les boutons de don/prise anonyme), mais n'est vérifié **nulle part côté serveur** — la branche anonyme de `giveProxyAction` (form vide) et `acceptProxyAction`/`event_proxy_take` ne testent jamais `$event->getAnonymousProxy()`. Un membre connaissant l'URL peut donner/prendre une procuration anonyme même si l'AG a `anonymous_proxy=false`. Fix : ajouter la vérification du flag dans les deux actions. Tracé en [TODO-PRIORISEE](../TODO-PRIORISEE.md) `m-SEC-12` (cf. [EXTRA](EXTRA.md)).
