...
class MembershipController extends AbstractController
{
    ...
    public function showAction(Membership $member, MembershipService $membership_service)
    {
        ...
        if ($this->get('security.authorization_checker')->isGranted('ROLE_ADMIN')) {
            ...
            $action = $this->generateUrl('member_new_registration', ['member_number' => $member->getMemberNumber(), 'token' => $member->getTmpToken($session->get('token_key') . $this->getCurrentAppUser()->getUsername())]);
        }
    ...
    public function editFirewallAction(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $member = null;
            if ($username) {
                $member = $em->getRepository('App:User')->findOneBy(['username' => $username]);
            } elseif ($member_number) {
                $member = $em->getRepository('App:Membership')->findOneBy(['member_number' => $member_number]);
            }
    ...
    public function activeUserAccountAction(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $member_number = $form->get('member_number')->getData();
            $em = $this->getDoctrine()->getManager();
            $ms = $em->getRepository('App:Membership')->findOneBy(['member_number' => $member_number]);
    ...
    public function newAction(Request $request, EventDispatcherInterface $event_dispatcher, SwipeCardHelper $swipeCardHelper)
    {
        ...
        // init member_number
        $m = $em->getRepository('App:Membership')->findOneBy([], ['member_number' => 'DESC']);
        $mm = 1;
        if ($m) {
    ...
    public function officeToolsAction(Request $request)
    {
        ...
        if ($note_form->isSubmitted()) {
            if ($note_form->isValid()) {
                $existing_note = $em->getRepository('App:Note')->findOneBy(['subject' => null, 'author' => $this->getCurrentAppUser(), 'text' => $note->getText()]);
                if ($existing_note) {
        ...
        $notes = $em->getRepository('App:Note')->findBy(['subject' => null]);
        $noteEditForms = [];
        $noteDeleteForms = [];
        $new_notes_form = [];
        foreach ($notes as $n) {
    ...
    private function redirectToShow(Membership $member)
    {
        ...
        return $this->redirectToRoute('member_show', ['member_number' => $member->getMemberNumber(), 'token' => $member->getTmpToken($session->get('token_key') . $this->getCurrentAppUser()->getUsername())]);
    }
