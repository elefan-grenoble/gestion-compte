            ...
            if ($closingException) {
                ...
                foreach ($periods as $period) {
                    ...
                    foreach ($period->getPositions() as $position) {
                        ...
                        $already_generated = $this->em->getRepository(Shift::class)->findBy(['start' => $start, 'end' => $end, 'job' => $period->getJob(), 'position' => $position]);
                        if (!$already_generated) {
                            $lastStart = $this->lastCycleDate($start);
                            $lastEnd = $this->lastCycleDate($end);
                            $last_cycle_shift = $this->em->getRepository(Shift::class)->findOneBy(['start' => $lastStart, 'end' => $lastEnd, 'job' => $period->getJob(), 'position' => $position]);
