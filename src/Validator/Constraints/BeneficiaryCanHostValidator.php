    ...
    public function validate($value, Constraint $constraint)
    {
        ...
        if (!$value->getMembership()) {
            $this->context->buildViolation($constraint->message)
                ...
                ->addViolation()
            ;
        } elseif ($value->getMembership()->isWithdrawn()) {
            $this->context->buildViolation($constraint->message)
                ->setParameter('{{ host }}', '#' . $value->getMemberNumber() . ' de ' . $value->getFirstname() . ' ' . $value->getLastname())
                ->setParameter('{{ reason }}', 'Son compte est fermé')
                ->addViolation()
            ;
        } elseif (!$this->memberService->isUptodate($value->getMembership())) {
            $this->context->buildViolation($constraint->message)
                ->setParameter('{{ host }}', '#' . $value->getMemberNumber() . ' de ' . $value->getFirstname() . ' ' . $value->getLastname())
                ->setParameter('{{ reason }}', 'Son compte est n\'est plus à jour d\'adhésion')
                ->addViolation()
            ;
        } elseif ($value->getMembership()->getBeneficiaries()->count() >= $this->maximum_nb_of_beneficiaries_in_membership) {
            $this->context->buildViolation($constraint->message)
                ->setParameter('{{ host }}', '#' . $value->getMemberNumber() . ' de ' . $value->getFirstname() . ' ' . $value->getLastname())
