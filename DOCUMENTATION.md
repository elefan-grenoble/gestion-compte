...
>
> **Sources** : sections D.1-5, CONFIG.1-3, LOG.1-3, SPEC.1-11 de `AUDIT.md` (audit juin 2026). En cas de divergence avec le `README.md` actuel, ce document fait foi — le README comporte des inexactitudes recensées en [§4](#4-installation--exploitation).
...
> ⚠️ **`CYCLE_DURATION` n'est pas honoré par `MembershipService`** : 6 emplacements (lignes 75, 146, 147, 155-156, 170, 181) hardcodent `28`. Le bug est **dormant tant que `CYCLE_TYPE=abcd`** (branche dédiée qui calcule depuis la semaine ISO) ; il devient actif sur une instance en cycles flottants avec une durée ≠ 28 jours.
...
| **Cycle** | `CYCLE_DURATION` ignoré (28 hardcodé ×6) — dormant si `abcd` | [§5.3](#53-paramètres-métier--valeurs-et-unités) |
