...
describe('super admin can freeze and unfreeze user', function () {
    it('super admin path', function () {
        ...
        cy.get('[data-cy=open_freeze_member_confirmation_modal]', { timeout: 5000 }).should('be.visible').click()

        cy.log('wait for modal to open, then confirm')
        cy.get('[data-cy=freeze_member_confirmation_modal_confirm]', { timeout: 5000 }).should('be.visible').click()
        ...
        cy.get('[data-cy=open_unfreeze_member_confirmation_modal]', { timeout: 5000 }).should('be.visible').click()

        cy.log('wait for modal to open, then confirm')
        cy.get('[data-cy=unfreeze_member_confirmation_modal_confirm]', { timeout: 5000 }).should('be.visible').click()
